    ...
    <nav className="fixed top-0 left-0 right-0 bg-white/80 dark:bg-gray-900/95 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800 z-50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-3 flex-1 justify-center">
          ...
          <Link href="/demo" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
            {t('nav.demo')}
          </Link>
          <Link href="/teammates" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
            {t('nav.teammates')}
          </Link>
          <Link href="/extension" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
            {t('nav.extension')}
          </Link>
          <Link href="/subscriptions" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
            {t('nav.subscriptions')}
          </Link>
          {user && (
            <>
              <Link href="/analysis" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
                {t('nav.player_analysis', { defaultValue: 'Player Analysis' })}
              </Link>
              <Link href="/dashboard" className="text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-gray-900 dark:hover:text-white transition-colors">
                {t('nav.dashboard')}
              </Link>
              {user.is_admin && (
                <Link
